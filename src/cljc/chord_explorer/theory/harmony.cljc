...
(defn diatonic-chord-type
  ...
  ([degree scale-type {:keys [seventh?] :or {seventh? false}}]
   (if seventh?
     (get-in diatonic-sevenths [scale-type degree]
             (get-in diatonic-sevenths [:major degree]))
     (get-in diatonic-triads [scale-type degree]
             (get-in diatonic-triads [:major degree])))))
